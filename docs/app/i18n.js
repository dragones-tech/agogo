  ...
  messages: {
    en: {
      nav: {
        ...
        activate: 'Activate modules', otw: 'Over the wire', config: 'Configuration',
      },
      home: {
      ...
      activate: {
        ...
        lead: 'Wiring a module is one line in <code>app.Use(...)</code>. The starter ships <code>logs</code>, <code>home</code> and <code>site</code> active, and <code>paginas</code> as a commented example. Two more from the framework: <code>logs</code> and <code>oauth</code>.',
        logsT: 'logs — access logging',
        logsD: 'A global middleware that logs each request (method, path, status, duration). Add <code>logs.Module()</code> to <code>app.Use</code> and that\'s it; remove the line and the code leaves the binary.',
        oauthT: 'oauth — OAuth 2.0 login',
        oauthD: 'Authorization-code login in pure stdlib that reuses the core <code>identity</code> service: it verifies with an external provider and then calls <code>identity.Login</code> to mark the session. Configured by its own env (<code>OAUTH_CLIENT_ID</code>, <code>OAUTH_AUTH_URL</code>, <code>OAUTH_TOKEN_URL</code>, <code>OAUTH_USERINFO_URL</code>, <code>OAUTH_SCOPE</code>); unconfigured, <code>/oauth/login</code> answers 503. Routes: <code>/oauth/login</code>, <code>/oauth/callback</code>, <code>/oauth/me</code> (protected).',
        note: 'These example modules aren\'t in the starter; the code is in git history. The point: a new login mechanism reuses <code>identity</code> without rewriting anything.',
      },
      otw: {
        title: 'Over the wire',
        lead: 'Two patterns that keep rendering on the server and ship HTML, not JSON — the agogo way of doing rich UX without an SPA framework.',
        partialT: 'Partial navigation (Turbo-style)',
        partialD: 'On a link click, the client asks the server for just the content fragment (the <code>X-Fragment</code> header → only the <code>content</code> block) and swaps <code>&lt;main&gt;</code>; header, footer and CSS stay. With <code>document.startViewTransition</code> for a native crossfade. Degrades cleanly: without JS (or on a non-HTML response) it\'s a normal full navigation, so SEO and robustness don\'t depend on the JS.',
        bffT: 'BFF: HTML over the wire from a token-gated API',
        bffD: 'When the data is behind a secret you can\'t expose (an external API gated by a token), the server calls it with the token (which never reaches the browser), renders the JSON as an HTML fragment, and returns that HTML; the client just does <code>innerHTML</code>. The concrete example fetches a GitHub repo\'s stats.',
        note: 'Not for your own DB — there you render the fragment directly from the query, no internal HTTP call.',
      },
      api: {
        title: 'API reference',
    ...
    es: {
      nav: {
        ...
        activate: 'Activar módulos', otw: 'Over the wire', config: 'Configuración',
      },
      home: {
      ...
      activate: {
        ...
        lead: 'Acoplar un módulo es una línea en <code>app.Use(...)</code>. El starter trae <code>logs</code>, <code>home</code> y <code>site</code> activos, y <code>paginas</code> como ejemplo comentado. Dos más del framework: <code>logs</code> y <code>oauth</code>.',
        logsT: 'logs — log de acceso',
        logsD: 'Un middleware global que registra cada petición (método, ruta, status, duración). Añade <code>logs.Module()</code> a <code>app.Use</code> y listo; quita la línea y el código sale del binario.',
        oauthT: 'oauth — login OAuth 2.0',
        oauthD: 'Login por authorization-code en stdlib pura que reusa el servicio <code>identity</code> del núcleo: verifica con un proveedor externo y luego llama a <code>identity.Login</code> para marcar la sesión. Se configura por su propio entorno (<code>OAUTH_CLIENT_ID</code>, <code>OAUTH_AUTH_URL</code>, <code>OAUTH_TOKEN_URL</code>, <code>OAUTH_USERINFO_URL</code>, <code>OAUTH_SCOPE</code>); sin configurar, <code>/oauth/login</code> responde 503. Rutas: <code>/oauth/login</code>, <code>/oauth/callback</code>, <code>/oauth/me</code> (protegida).',
        note: 'Estos módulos de ejemplo no están en el starter; el código vive en el historial de git. La idea: un mecanismo de login nuevo reusa <code>identity</code> sin reescribir nada.',
      },
      otw: {
        title: 'Over the wire',
        lead: 'Dos patrones que mantienen el render en el servidor y envían HTML, no JSON — la forma agogo de hacer UX rica sin un framework SPA.',
        partialT: 'Navegación parcial (estilo Turbo)',
        partialD: 'Al hacer clic en un enlace, el cliente le pide al servidor solo el fragmento de contenido (la cabecera <code>X-Fragment</code> → solo el bloque <code>content</code>) y reemplaza <code>&lt;main&gt;</code>; header, footer y CSS quedan fijos. Con <code>document.startViewTransition</code> para un crossfade nativo. Degrada limpio: sin JS (o ante una respuesta no-HTML) es una navegación normal, así SEO y robustez no dependen del JS.',
        bffT: 'BFF: HTML over the wire desde una API con token',
        bffD: 'Cuando el dato está detrás de un secreto que no puedes exponer (una API externa protegida por token), el servidor la llama con el token (que nunca llega al navegador), renderiza el JSON como fragmento HTML, y devuelve ese HTML; el cliente solo hace <code>innerHTML</code>. El ejemplo concreto trae las estadísticas de un repo de GitHub.',
        note: 'No para tu propia BD — ahí renderizas el fragmento directo desde la query, sin HTTP interno.',
      },
      api: {
